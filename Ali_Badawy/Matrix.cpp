...
Matrix* Matrix::atanMatrix(){
 ...
 Matrix* Matrix::naturalLogMatrix(){
	Matrix *temp = new Matrix;
	if(this->notMatrix){
        temp->setNum(log(this->num));
	}
	temp -> setSize(this->rows,this->columns);
        for(unsigned int i = 0;i<rows;i++)
            for(unsigned int j = 0; j<columns;j++){
				if(temp->twoDArray[i][j]==0){throw("math error");}
                else {
                        temp->twoDArray[i][j] = log(this->twoDArray[i][j]);

                }
                }
	return temp;
}
 Matrix* Matrix::LogMatrix(float base){
	Matrix *temp = new Matrix;
	if(this->notMatrix){
        temp->setNum(log(this->num)/log(base));
	}
	temp -> setSize(this->rows,this->columns);
        for(unsigned int i = 0;i<rows;i++)
            for(unsigned int j = 0; j<columns;j++){
				if(temp->twoDArray[i][j]==0){throw("math error");}
                        ...
                        temp->twoDArray[i][j] = (log(this->twoDArray[i][j])/log(base));

                }
                }
	return temp;
}
 Matrix* Matrix::gammaMatrix(){
	Matrix *temp = new Matrix;
	if(this->notMatrix){
        temp->setNum(tgamma(this->num));
	}
	temp -> setSize(this->rows,this->columns);
        for(unsigned int i = 0;i<rows;i++)
            for(unsigned int j = 0; j<columns;j++){
                        temp->twoDArray[i][j] = (tgamma(this->twoDArray[i][j]));
                }
 ...
 Matrix* Matrix::expMatrix(){
	Matrix *temp = new Matrix;
	if(this->notMatrix){
        temp->setNum(exp(this->num));
	}
	temp -> setSize(this->rows,this->columns);
        for(unsigned int i = 0;i<rows;i++)
            for(unsigned int j = 0; j<columns;j++){
                        temp->twoDArray[i][j] = (exp(this->twoDArray[i][j]));
                }
