...
Matrix multiOpHandling(string RHS, vector<Matrix>& storedMatrices, vector<string>& systemCommands)
{
	...
	temporaryName = "temp11"; //m3mlthash 00 3shan el 0 e3tbrha ely hya bta3t el termination de wtala3 error

	if (RHS.find("+") == RHS.npos && RHS.find("-") == RHS.npos && RHS.find("*") == RHS.npos && RHS.find("/") == RHS.npos && RHS.find("inv") == RHS.npos && RHS.find("'") == RHS.npos && RHS.find("`") == RHS.npos && RHS.find("det") == RHS.npos)
	{
		for (size_t i = 0; i < storedMatrices.size(); i++)
		{
	...
	while (RHS.find(".+") != RHS.npos || RHS.find(".-") != RHS.npos || RHS.find(".*") != RHS.npos || RHS.find("./") != RHS.npos)
	{
		...
		if (operands[0].find_first_not_of(alphabets) != operands[0].npos  && operands[0].find("temp")==operands[0].npos)
		{
			char *e;
			double value = strtod(operands[0].c_str(), &e);
			if (*e != 0) throw("Invalid number.");
		...
		if (operands[1].find_first_not_of(alphabets) != operands[1].npos && operands[1].find("temp") == operands[1].npos)
		{
			char *e;
			double value = strtod(operands[1].c_str(), &e);
			if (*e != 0) throw("Invalid number.");
		...
		RHS.erase(opIndex - operands[0].length(), operands[0].length() + operands[1].length() + 2);
	...
	while (RHS.find("+") != RHS.npos || RHS.find("-") != RHS.npos || RHS.find("*") != RHS.npos || RHS.find("/") != RHS.npos)
	{
		...
		RHS.erase(opIndex - operands[0].length(), operands[0].length() + operands[1].length() + 1);
		RHS.insert(opIndex - operands[0].length(), temporaryName);
		temp->setName(temporaryName);
		storedMatrices.push_back(*temp);
		if (temporaryName[5] < '9')temporaryName[5]++;
