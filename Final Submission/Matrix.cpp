...
Matrix* Matrix::sqrtMatrix() {
	Matrix *temp = new Matrix;
	if (this->notMatrix) {
		if (this->num < 0)throw("can't find sqrt of negative value");
...
Matrix* Matrix::tanMatrix() {
	...
	if (this->notMatrix) {
		...
		for (unsigned int i = 0; i<rows; i++)
			for (unsigned int j = 0; j<columns; j++) {
				if (this->twoDArray[i][j] == PI / 2.0) throw ("can't find tan of pi/2 (undefined value)");
...
Matrix* Matrix::asinMatrix() {
	Matrix *temp = new Matrix;
	if (this->notMatrix) {
		if (this->num > 1)throw("can't find asin of number bigger than 1 (undefined value)");
		...
		for (unsigned int i = 0; i<rows; i++)
			for (unsigned int j = 0; j < columns; j++)
			{
				if(this->twoDArray[i][j]> 1)throw("can't find asin of number bigger than 1 (undefined value)");
...
Matrix* Matrix::acosMatrix() {
	Matrix *temp = new Matrix;
	if (this->notMatrix) {
		if (this->num > 1)throw("can't find acos of number bigger than 1 (undefined value)");
		...
		for (unsigned int i = 0; i<rows; i++)
			for (unsigned int j = 0; j < columns; j++)
			{
				if (this->twoDArray[i][j] > 1)throw("can't find acos of number bigger than 1 (undefined value)");
...
Matrix Matrix::multiOpHandling(string RHS, vector<Matrix>& storedMatrices, vector<string>& systemCommands, bool &print)
{
	...
	while (RHS.find('^') != RHS.npos)
	{
		...
		else if ((*parameter1).notMatrix && (*parameter2).notMatrix) *temp = pow((*parameter1).num, (*parameter2).num);
		else
		{
			if (op == ".^")
			{
			...
			{
				if (((!firstNo) && secondNo)||(!(*parameter1).notMatrix&& (*parameter2).notMatrix))
				{
					double No;
					if (secondNo)No = sNo;
					else if ((*parameter2).notMatrix)No = (*parameter2).num;
					if (No != (int)No)throw("not supported: raise matrix to fraction power");
					Matrix *powResult = new Matrix;
					*powResult = *parameter1;
					if (sNo == 0)
					{
						if ((*parameter1).getRows() != (*parameter1).getColumns())throw("can't raise non square matrix to power of 0");
						(*temp).eye((*parameter1).getRows(), (*parameter1).getColumns());
					}
					else
					{
						for (size_t i = 1; i < sNo; i++)
						{
							*powResult = *powResult*(*parameter1);
						}
						temp = powResult;
					}
				}
	...
	while (RHS.find("+") != RHS.npos || RHS.find("-") != RHS.npos || RHS.find("*") != RHS.npos || RHS.find("/") != RHS.npos)
	{
		...
		if (opIndex < 0)throw("invalid mathematical expression");
		RHS.erase(opIndex - operands[0].length(), operands[0].length() + operands[1].length() + op.length());
		RHS.insert(opIndex - operands[0].length(), temporaryName);
